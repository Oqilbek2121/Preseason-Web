...
app.get("/", (req, res) => {
    let sinatraMusicSongs = ["That's Life", "Fly Me to the Moon", "My Way", "New York", "New York", "Strangers in The Night",
    ...
    let f = Math.floor(Math.random() * sinatraMusicSongs.length);
    let musicSong = sinatraMusicSongs[f];
    res.send(musicSong);
});
...
app.use("/protected", (req, res, keyingi) => {
    const user = {
      login: 'admin',
      password: 'admin'
    }
    ...
    if (login && password && login === user.login && password === user.password) {
